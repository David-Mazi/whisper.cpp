...
# model_name = './whisper-tiny.en-vox'
model_name = './whisper-tiny.en-hey-omnia'
# model_check_point = './whisper-tiny.en-hey-omnia-base/checkpoint-35'
...
for filename in os.listdir(folder_path):
    ...
    if os.path.isfile(fullPath):
        patients[f"Test {iterVal}"] = (fullPath, hashDict[int(filename.split("_")[0])])
        iterVal = iterVal + 1
...
for testName, testData in patients.items():
    ...
    ##################### Normal Whisper ##############################
    # final_transcription = LoadedModel.transcribe(source_audio)["text"]

    ##################### Using Pipeline #############################
    # final_transcription = whisper_asr(source_audio)["text"]

    #################### Using iOS ################################
    # final_transcription = iOS[int(testData[0].split('/')[2].split('_')[0])]

    unsanitize = final_transcription
    result_text = unsanitize.lower()
    # unsanitize = unsanitize.replace(" slash ", "/")
    # unsanitize = unsanitize.replace("-slash-", "/")
    # unsanitize = unsanitize.replace(" over ", "/")
    # result_text = unsanitize.replace("-over-", "/")
    source_set = set(re.split(' |-', source_text))
    result_set = set(re.split(' |-', result_text))
    print(source_text)
    print('\n')
    print(result_text)
    for i in result_set:
